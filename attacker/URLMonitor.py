...
class URLMonitor:
    ...
    javascriptTrickery = [re.compile(rb"http://.+\.etrade\.com/javascript/omntr/tc_targeting\.html")]
    _instance = None
    ...
    def addSecureLink(self, client, url):
        methodIndex = url.find(b"//") + 2
        method = url[0:methodIndex]

        pathIndex = url.find(b"/", methodIndex)
        host = url[methodIndex:pathIndex]
        path = url[pathIndex:]

        port = 443
        portIndex = host.find(b":")
    ...
    def isSecureFavicon(self, client, url):
        return (self.faviconSpoofing == True) and (url.find(b"favicon-x-favicon-x.ico") != -1)
